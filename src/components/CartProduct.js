      ...
      <div className="flex items-center">
        <div className="flex items-center space-x-3">
          <button
            ...
            className="p-1 rounded-md hover:bg-rose-100 hovre:text-rose-500"
          >
          ...
          <button
            onClick={() => setItemQuantity(product.id, product.quantity + 1)}
            className="p-1 rounded-md hover:bg-green-100 hovre:text-green-500"
          >
        ...
        <button onClick={() => removeItem(product.id)} className="ml-4 hover:text-red-500">
          <XCircleIcon className="w-6 h-6 flex-shrink-0 opacity-500 hover:opacity-100 transition-opacity" />
        </button>
      </div>
    </div>
  );
